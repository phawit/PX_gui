...
Ui_CustomWindow, QtBaseClass = uic.loadUiType('ui/custom.ui')
Ui_AddCustomWindow, QtBaseClass = uic.loadUiType('ui/addCustom.ui')
...
class Barcode_process:
    def __init__(self) :
        ...
        with open('db/config.csv','r') as f:
            for line in f:
                barcode_root_path = line.strip().split('\n')[0]
        
        self.dev = InputDevice(barcode_root_path)
            ...
            while True:
                ...
                for event in self.dev.read():
                    if event.type == 1 and event.value == 1:
                        barcode_tmp += (self.keys[event.code])
                if (len (barcode_tmp)) > 13:
                    break
            if barcode_tmp != "":
                self.barcode.append(barcode_tmp[:-1])
            # self.barcode.append(input("Scan Barcode: "))
...
class Custom_window(QtGui.QDialog, Ui_CustomWindow):
    def __init__(self, parent=None):
        super(Custom_window, self).__init__(parent)
        self.setupUi(self)

        self.selectedRow = -1
        self.table_stock1.clicked.connect(self.updateSelected1)
        ...
        # self.WD_app = WD_window(self)
        self.addCustom_app = AddCustom_window(self)
        # self.historyCustom_app = HistoryCustom_window(self)
        
        # self.WD_button.clicked.connect(self.addStock)
        self.addCustom_button.clicked.connect(self.addCustom)
        # self.removeCustom_button.clicked.connect(self.OpenStockHistory)
        # self.historyCustom_button.clicked.connect(self.removeItems)

        


        header = ['ID', 'ยศชื่อ-สกุล', 'จำนวนเงิน', 'xx']
        header = [ unicode(h, "utf-8") for h in header]
        self.table_stock1.setHorizontalHeaderLabels(header)
        header = self.table_stock1.horizontalHeader()
        header.setResizeMode(1, QtGui.QHeaderView.Stretch)
        header.setResizeMode(0, QtGui.QHeaderView.ResizeToContents)
        header.setResizeMode(2, QtGui.QHeaderView.ResizeToContents)
        header.setResizeMode(3, QtGui.QHeaderView.ResizeToContents)
        ...
        for custom in custom_list:
            if int(custom['Stock']) > -1:
                custom_array = [custom['Custom ID'], custom['Custom Name'], custom['Custom Money'], custom['Custom Stock']]
                r = self.table_stock1.rowCount()
                self.table_stock1.insertRow(r)
                for i in range(len(custom_array)):
                    self.table_stock1.setItem(r , i, QtGui.QTableWidgetItem(custom_array[i]))
                    if i > 1:
                        self.table_stock1.item(r, i).setTextAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)
        self.table_stock1.sortItems(1)
    ...
    def addCustom(self):
        self.addCustom_app.line_id.setText('')
        self.addCustom_app.line_name.setText('')
        self.addCustom_app.line_money.setText('')
        self.addCustom_app.line_units.setText('')
        self.addCustom_app.line_id.setFocus(True)
        self.addCustom_app.show()
        self.addCustom_app.activateWindow()

    def updateSelected1(self):
        self.selectedRow = self.table_stock1.currentRow()
...
# --------------------Ui_AddCustomWindow, QtBaseClass = uic.loadUiType('ui/addCustom.ui')
class AddCustom_window(QtGui.QDialog, Ui_AddCustomWindow):
    def __init__(self, parent=None):
        super(AddCustom_window, self).__init__(parent)
        self.setupUi(self)
        ...
        self.add_button.clicked.connect(self.AddCustom)
        self.line_id.textChanged.connect(self.BarcodeChange)

    def BarcodeChange(self):
        for items in items_list:
            if self.line_id.text() == custom['Custom ID']:
               self.line_name.setText(custom['Custom Name'])
               self.line_money.setText(custom['Custom Money'])
               self.line_units.setText(custom['Custom Stock'])
            #    custom_array = [custom['Custom ID'], custom['Custom Name'], custom['Custom Money'], custom['Custom Stock']]
                


    def WriteTable2CSV1(self):
        file = open('db/CustomList.csv','w')
        fcsv = csv.writer(file, lineterminator='\n')
        for custom in custom_list:
            info = [custom['Custom ID'], custom['Custom Name'].encode('utf-8'), custom['Custom Money'], custom['Custom Stock']]
            fcsv.writerow(info)
        file.close()
    ...
    def AddCustom(self):
        ...
        if str(self.line_units.text()) == '':
            ...
            self.parent().table_stock1.setSortingEnabled(False)
            inList = False
            out_of_stock = False
            for custom in custom_list:
                if custom['Custom ID'] == str(self.line_id.text()):
                    if int(custom['Custom Stock']) == -1:
                        out_of_stock = True
                        custom['Custom Stock'] = '0'
                    inList = True
                    break
            
            if inList:
                if out_of_stock:
                    r = self.parent().table_stock1.rowCount()
                    while r > 0:
                        self.parent().table_stock1.removeRow(r-1)
                        r = self.parent().table_stock1.rowCount()
                    ...
                    for custom in custom_list:
                        if int(custom['Custom Stock']) > -1:
                            custom_array = custom['Custom ID'], custom['Custom Name'], custom['Custom Money'], custom['Custom Stock']
                            r = self.parent().table_stock1.rowCount()
                            self.parent().table_stock1.insertRow(r)
                            for i in range(len(custom_array)):
                                self.parent().table_stock1.setItem(r , i, QtGui.QTableWidgetItem(custom_array[i]))
                                if i > 1:
                                    self.parent().table_stock1.item(r, i).setTextAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)

                for ii in range(len(items_list)):
                    custom = custom_list[ii]
                    if custom['Custom ID'] == str(self.line_id.text()):
                        for rr in range(self.parent().table_stock1.rowCount()):
                            if str(self.line_id.text()) == str(self.parent().table_stock1.item(rr, 0).text()):
                                num_stock = int(str(self.parent().table_stock1.item(rr, 3).text())) + int(str(self.line_units.text()))
                                self.parent().table_stock1.setItem(rr, 3, QtGui.QTableWidgetItem(str(num_stock)))
                                self.parent().table_stock1.item(rr, 3).setTextAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)
                                custom['Custom Stock'] = str(num_stock)
                                UpdateJSON('stock', [custom['Custom ID'], int(str(self.line_units.text()))])
                    custom_list[ii] = items
             #    custom_array = [custom['Custom ID'], custom['Custom Name'], custom['Custom Money'], custom['Custom Stock']]
            else:
                custom = custom_info.copy()
                custom['Custom ID'] = str(self.line_id.text())
                custom['Custom Name'] = unicode(self.line_name.text())
                custom['Custom Money'] = "{:.2f}".format(float(self.line_money.text()) )
                custom['Custom Stock'] = str(self.line_units.text())
                custom_list.append(custom)
                UpdateJSON('stock', [custom['Custom ID'], int(str(self.line_units.text()))])
            
            print 'write table'
            self.WriteTable2CSV1()
            ...
            for custom in custom_list:
                if int(custom['Custom Stock']) > -1:
                    ...
                    for i in range(len(custom_array)):
                        ...
                        if i > 1:
                            self.parent().table_stock1.item(r, i).setTextAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)
            self.parent().table_stock1.setSortingEnabled(True)
            self.parent().table_stock1.sortItems(1)
            self.parent().selectedRow = -1
            self.close()
...
class DateHistory_window(QtGui.QDialog, Ui_DateHistoryWindow):
    ...
    def show_detail(self):
        ...
        detail = str(self.table_sales.item(row, 0).text())
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.custom_app = Custom_window(self)
        ...
        self.custom_button.clicked.connect(self.OpenCustomWindow)
    ...
    def OpenCustomWindow(self):
        ...
        del self.custom_app
        self.custom_app = Custom_window(self)
        self.custom_app.show()
        self.custom_app.activateWindow()
        # x=1
...
if __name__ == "__main__":
    ...
    print 'main'
    with open('db/CustomList.csv','r') as f:
        for line in f:
            line_split = line.strip().split(',')
            (customID,customName,customMoney,customStock) = line_split
            custom = custom_info.copy()
            custom['Custom ID'] = customID
            custom['Custom Name'] = unicode(customName, "utf-8")
            custom['Custom Money'] = customMoney
            custom['Custom Stock'] = customStock
            custom_list.append(custom)
