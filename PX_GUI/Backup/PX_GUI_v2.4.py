...
class AddStock_window(QtGui.QDialog, Ui_AddStockWindow):
    ...
    def AddStock(self):
        ...
        if not inList:
            ...
            items['Item Name'] = self.line_name.text()
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def save_shortcut_button(self):
        file_sb = open('sb.csv','w')
        for sb in self.shortcut_button:
            barcode = str(sb.text().split('\n')[1])
            file_sb.write(barcode + '\n')
        file_sb.close()

    def load_shortcut_button(self):
        with open('sb.csv','r') as f:
            for line in f:
                barcode = line.strip().split('\n')[0]
                for items in items_list:
                    if barcode == items['Barcode']:
                        row = self.table_shortcut.rowCount()
                        col = self.num_shortcut_button % self.button_per_row
                        if col == 0:
                            self.table_shortcut.insertRow(row)
                        else:
                            row -= 1

                        self.shortcut_button.append(QLabel(items['Item Name'] + '\n' + items['Barcode'] + '\n' + items['Price'] + u' บาท', self))
                        self.shortcut_button[self.num_shortcut_button].setGeometry(QtCore.QRect(70, 80, 300, 100)) 
                        self.shortcut_button[self.num_shortcut_button].setAlignment(Qt.AlignCenter)
                        if (row+col)%2 == 0:
                            self.shortcut_button[self.num_shortcut_button].setStyleSheet("background-color: rgb(135, 255, 157)")
                        else:
                            self.shortcut_button[self.num_shortcut_button].setStyleSheet("background-color: rgb(255, 167, 135)")
                        self.table_shortcut.setCellWidget(row, col, self.shortcut_button[self.num_shortcut_button] )
                        self.table_shortcut.resizeRowsToContents()
                        self.table_shortcut.resizeColumnsToContents()
                        self.num_shortcut_button += 1
